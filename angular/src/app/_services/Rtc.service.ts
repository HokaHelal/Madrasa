...
export class RtcService {
  ...
  public createPeer(stream, userId: string, initiator: boolean): Instance {
    ...
    peer.on('signal', data => {
      ...
      this.onSignalToSend.next({ connectionId: userId, stream: stringData });
    });

    peer.on('stream', data => {
      console.log('on stream', data);
      this.onStream.next({ connectionId: userId, stream: data });
    });

    peer.on('connect', () => {
      this.onConnect.next({ connectionId: userId, stream: null });
    });

    peer.on('data', data => {
      this.onData.next({ connectionId: userId, stream: data });
    });
